...
##                esim erikoiskartoille oma .csv -tiedosto
...
# (this can be usefull, if there are a lot of maps in the configuration csv) 
    with open("makekap.BAT", "a") as bat_file:
        print("magick "+map+".png -remap palette.png "+map+"_c128.png", file=bat_file) ## Yhtenainen varipaletti
        print("imgkap "+map+"_c128.png "+map+".txt", file=bat_file)                    ## Geokoodaus ja PNG>KAP konversio
        print("copy "+map+"_c128.kap 128_Colors", file=bat_file)                       ## Kopioi alihakemistoon (huom. hakemisto oltava olemassa)
...
def Download_Tile(link):
    ...
    link_x = link.split('tilecol=')[1].split('&')[0]
    link_y = link.split('tilerow=')[1].split('&')[0]
    link_z = link.split('WGS84_Pseudo-Mercator:')[1].split('&')[0]
    print("WMTS_"+wmtsName+"_"+link_z+"_"+link_x+"_"+link_y+".png")
    while retryAttemptsRemaining > 0:
        try:
            ##Get one tile from wmts and save bitmap as WMTS_mapDataName_zoom_x_y.png
            request.urlretrieve(link, "WMTS_"+wmtsName+"_"+link_z+"_"+link_x+"_"+link_y+".png")
...
def confirmLargeFile(maxTiles, filename, xmin, xmax, ymin, ymax, zoomLevel):
...
## OpenCPN tuntuu toimivan hyvin 2000 (256X256) tiilesta koostuvien karttojen kanssa. Ei tiedossa miten isoja kartat voisivat olla.
...
def merge_images( zoom, xmin, xmax, ymin, ymax, output_file) :
    print("Rakennetaan kartta: "+output_file)
    TILESIZE = 256
    ...
    for x in range(xmin, xmax+1) :
        imy = 0
        for y in range(ymin, ymax+1) :
            #tile = Image.open( "WMTS_"+"%s_%s_%s_%s.png" % (wmtsName, zoom, x, y) )
            #print("WMTS_"+wmtsName+"_"+str(zoom)+"_"+str(x)+"_"+str(y)+".png")
            tile = Image.open( "WMTS_"+wmtsName+"_"+str(zoom)+"_"+str(x)+"_"+str(y)+".png")
...
def main() :
    global wmtsName ## "Tilapaisten" WMTS tiedostojen linkkaus aneistoihin (ettei tule saman nimisia tiedostoja eri tarkoituksessa)
    if (len(sys.argv) - 1) != 1:
        print("Anna parametriksi tiedostonimi")
        exit()
    else:
        gridData = pandas.read_csv(sys.argv[1], na_filter='', dtype=str, sep=';', comment='#')
        wmtsName = gridData['mapData'][0] ##### Oletetaan, etta kasittelyssa on tiilia vain yhdesta aineistosta
        wmtsName = wmtsName.replace('%20', "_") ## korvataan url-valilyonti alaviivalla
        wmtsName = wmtsName.replace(':', "_")   ## korvataan myos : alaviivalla
        for index, row in gridData.iterrows():
            ...
            #Alla oleva tarkistaa, etta hakuja ei ole liikaa ja varoittaa isosta karttalehdesta. Voi laittaa paalle uusia aineistoja tehdessa.
            confirmLargeFile(3000, row['fileName'], int(row['x_min']), int(row['x_max']), int(row['y_min']), int(row['y_max']), int(row['zoomLevel']))
